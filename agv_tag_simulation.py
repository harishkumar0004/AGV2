...
class AGVGridSimulator(QMainWindow):
    ...
    def build_ui(self):
        ...
        self.click_block_cb = QCheckBox("Click cell toggles blocked cell")
        ...
        self.click_block_cb.clicked.connect(lambda: self.set_click_mode("block"))

        click_layout.addWidget(self.click_goal_cb)
        click_layout.addWidget(self.click_start_cb)
        click_layout.addWidget(self.click_block_cb)
    ...
    def set_click_mode(self, mode):
        ...
        self.click_block_cb.setChecked(mode == "block")
    ...
    def on_grid_clicked(self, tag):
        ...
        if mode == "block":
            ...
            if tag in self.blocked:
                self.blocked.remove(tag)
                self.append_log(f"Unblocked Tag {tag}")
            else:
                self.blocked.add(tag)
                self.append_log(f"Blocked Tag {tag}")
