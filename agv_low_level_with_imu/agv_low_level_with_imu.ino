...
// Higher = faster speed change.
// Lower = smoother but slower response.
const float ACCEL_PPS_PER_SEC = 7000.0f;
const float DECEL_PPS_PER_SEC = 9000.0f;
...
const int BASE_PPS = 5500;

const float KP_IMU_PPS_PER_DEG = 70.0f;
const int MAX_IMU_CORR_PPS = 250;
...
// If TURN_REL 90 rotates the wrong direction, change this to -1.
...
// Actual PPS currently applied to motors
int currentLeftPps = 0;
int currentRightPps = 0;

// Target PPS requested by controller / Raspberry Pi
int targetLeftPps = 0;
...
// ====================================================
// SET LEFT MOTOR SPEED IMMEDIATELY
// ====================================================
...
void setLeftPPS(uint32_t pps) {
  ...
  if (pps == 0) {
    ...
    // Current ISR creates square wave.
    // So timer is half-period.
...
// ====================================================
// SET RIGHT MOTOR SPEED IMMEDIATELY
// ====================================================
...
// ====================================================
// APPLY SIGNED LEFT PPS IMMEDIATELY
// ====================================================
...
// ====================================================
// APPLY SIGNED RIGHT PPS IMMEDIATELY
// ====================================================
...
// ====================================================
// UPDATE TRAPEZOID / VELOCITY RAMP
// ====================================================
...
void updateVelocityRamp() {
  ...
  int leftDelta = abs(targetLeftPps) < abs(currentLeftPps)
                    ? (int)(DECEL_PPS_PER_SEC * dt)
                    : (int)(ACCEL_PPS_PER_SEC * dt);

  int rightDelta = abs(targetRightPps) < abs(currentRightPps)
                    ? (int)(DECEL_PPS_PER_SEC * dt)
                    : (int)(ACCEL_PPS_PER_SEC * dt);
  ...
  int newLeft = rampToward(
    currentLeftPps,
    targetLeftPps,
    leftDelta
  );

  int newRight = rampToward(
    currentRightPps,
    targetRightPps,
    rightDelta
  );
...
// ====================================================
// SET WHEEL VELOCITY WITH RAMP TARGET
// ====================================================
...
void processCommand(const String& cmd) {
  ...
  // ------------------------------------------------
  // Direct command from Raspberry Pi
  // ------------------------------------------------
  ...
  // ------------------------------------------------
  // Lock current corrected heading and move straight
  // ------------------------------------------------
  ...
  else if (sscanf(cmd.c_str(), "TURN_REL %f", &turnDeg) == 1) {
...
void setup() {
  ...
  // T60 enable inputs are active LOW
  ...
  Serial.println("Ramp:");
  Serial.print("  ACCEL_PPS_PER_SEC=");
  Serial.println(ACCEL_PPS_PER_SEC);
  Serial.print("  DECEL_PPS_PER_SEC=");
  Serial.println(DECEL_PPS_PER_SEC);
}
