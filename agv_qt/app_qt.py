...
class AprilTagCameraThread(QThread):
    ...
    def _open_camera(self):
        # Use V4L2 to avoid GStreamer memory allocation issues on Raspberry Pi.
        cap = cv2.VideoCapture(self.camera_index, cv2.CAP_V4L2)

        if not cap.isOpened():
            self.status.emit("Camera open failed with CAP_V4L2")
            return None
        ...
        if not ok or frame is None:
            self.status.emit("Camera opened but frame read failed")
            cap.release()
            return None
    ...
    def run(self):
        ...
        if cap is None:
            self.status.emit("Camera unavailable")
            return
        ...
        while self.running:
            ...
            if detected_ids:
                self.status.emit("Detected: " + ", ".join(str(x) for x in sorted(set(detected_ids))))
            else:
