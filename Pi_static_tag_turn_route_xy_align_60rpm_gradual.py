#!/usr/bin/env python3
"""
Static AprilTag route runner with XY turn-tag centering.
...
Behavior:
  ...
  - tag 3 -> center tag using dx/dy, pre-turn forward, right 90 degree pivot,
    post-turn forward
  - tag 5 -> center tag using dx/dy, pre-turn forward, right 90 degree pivot,
    post-turn forward
  - tag 7 -> stop

Upload `04_Manual_debug_distance_angle_control_60rpm_gradual.ino` to the Mega before running this script.
"""
...
DEFAULT_CHECKPOINT_STABLE_FRAMES = 1
DEFAULT_TURN_STABLE_FRAMES = 1
...
def build_arg_parser():
    ...
    parser.add_argument("--turn-center-y-tolerance-px", type=float, default=DEFAULT_TURN_CENTER_Y_TOLERANCE_PX,
                        help="Turn tag is considered centered when abs(tag center y - image center y) is below this many pixels.")
    parser.add_argument("--turn-align-step-cm", type=float, default=DEFAULT_TURN_ALIGN_STEP_CM,
                        help="Small slow forward/backward step used while aligning to a turn tag.")
    ...
    parser.add_argument("--invert-turn-align-y", action="store_true",
                        help=("Reverse the forward/backward choice during turn-tag centering. "
                              "Default is based on your logs: dy > 0 uses BACK_CM, dy < 0 uses FWD_CM."))
...
def turn_tag_center_status(gate_info, args):
    dx_px = gate_info.get("dx_px")
    dy_px = gate_info.get("dy_px")
    if dx_px is None or dy_px is None:
        return False, False, False

    centered_x = abs(dx_px) <= args.turn_center_tolerance_px
    centered_y = abs(dy_px) <= args.turn_center_y_tolerance_px
    return centered_x, centered_y, centered_x and centered_y


def is_turn_tag_centered(gate_info, args):
    return turn_tag_center_status(gate_info, args)[2]


def choose_turn_align_motion(gate_info, args):
    dy_px = gate_info.get("dy_px")
    if dy_px is None:
        return "FWD_CM"

    # In the captured logs, moving forward made tag center_y increase.
    # So dy > 0 means the tag is already below image center: move backward.
    move_backward = dy_px > args.turn_center_y_tolerance_px
    move_forward = dy_px < -args.turn_center_y_tolerance_px

    if args.invert_turn_align_y:
        move_backward, move_forward = move_forward, move_backward
...
def add_route_fields(row, route_state, route_active, stable_tag_id, stable_count, route_action,
    ...
    row.update({
        ...
        "turn_center_y_tolerance_px": base.fmt(args.turn_center_y_tolerance_px),
...
def main():
    ...
    print(
        f"Turn centering: abs(dx) <= {args.turn_center_tolerance_px:.1f} px, "
        f"abs(dy) <= {args.turn_center_y_tolerance_px:.1f} px, "
        f"step {args.turn_align_step_cm:.1f} cm"
    )
    print("Turn Y align default: dy > 0 uses BACK_CM, dy < 0 uses FWD_CM")
